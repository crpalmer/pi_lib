...
class AxisStepperClock : public PhysicsClock {
public:
...
AxisStepper::AxisStepper(Output *dir, Output *step, double steps_per_mm) : steps_per_mm(steps_per_mm), mm_per_step(1.0 / steps_per_mm), dir(dir), step(step) {
    clock = new AxisStepperClock();
...
static inline unsigned physics_steps(AxisStepperClock *clock, Physics *physics, Output *step, double steps_per_mm, int n_steps, double target_v, double jerk, Input *end_stop = NULL) {
    double v = 0;
...
void AxisStepper::go(double target, double feed) {
    AxisStepperClock *clk = (AxisStepperClock *) clock;
...
void AxisStepper::home(Input *end_stop, bool direction, double homed_pos, double feed) {
    last_feed = feed;
    physics->set_max_velocity((feed > jerk) ? (feed - jerk) : jerk);
    physics->start_motion(0, 10*1000);

    dir->set(direction);
    physics_steps((AxisStepperClock *) clock, physics, step, steps_per_mm, 1000*1000, feed, jerk);
...
void AxisStepper::set_acceleration(double mm_per_sec_squared) {
    accel = mm_per_sec_squared;
    physics->set_acceleration(accel);
}

void AxisStepper::set_jerk(double jerk) {
